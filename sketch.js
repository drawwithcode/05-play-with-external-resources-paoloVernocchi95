...
function preload() {
  ...
  corpo = loadImage('assets/corpo2.png');
...
function draw() {
  ...
  for (var i = 0; i < spectrum.length; i++) {
    ...
    if (y > windowHeight/20*12 && y < windowHeight /20*14) { //premuti a metà
      fill(225);
    } else if (y > windowHeight / 20 * 12) { //non premuti
      fill(255);
    } else { // premuti
      fill(200);
    }
  ...
  for (var j = 0; j < spectrum.length; j+=50) {
    ...
    if (ang>10) {
      ...
      rotate(ang/2);
      image(bracciosotto, 0, 0, bracciosotto.width / 8, bracciosotto.height / 10);
      pop();

    }else if (ang>8 && ang<10) {
      push();
      translate( windowWidth / 4 + 35, windowHeight / 6 + 80);
      rotate(ang/2);
        image(bracciosopra,0, 0, bracciosopra.width / 8, bracciosopra.height / 10);
          pop();
} else {
  push();
    translate( windowWidth / 4 + 35, windowHeight / 6 + 80);
  rotate(5);
    ...
    image(bracciosotto, 0,0, bracciosotto.width / 8, bracciosotto.height / 10);
  pop();
    image(corpo, windowWidth / 4, windowHeight / 6, corpo.width / 9, corpo.height / 10);
    push();
      translate( windowWidth / 4 + 35, windowHeight / 6 + 80);
    rotate(10);
